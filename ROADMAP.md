...
| 1.3 | **Wire example-domain into infra and apps** | ✅ Done. Example apps (`@caf/example-react`, `@caf/example-vue`, `@caf/example-angular`) and `@caf/example-infrastructure` depend on `@caf/example-domain`. Infrastructure packages (`@caf/infrastructure-react`, `@caf/infrastructure-vue`, `@caf/infrastructure-angular`) no longer depend on example-domain - they are framework-agnostic. Core has no dependency on example-domain. |
...
| 5.4 | **Example / demo** | ? Done. Enhanced "Example / Demo" section in root README: highlights `@caf/example-domain` package (domain/application structure, entities, use cases), describes three demo apps (React/Vue/Angular), explains they share same domain logic, includes running instructions, and points to framework-specific implementations. |
...
| 7.6 | **Request interface** | ? Done. Created `IRequestHandler` interface and updated `ApiRequest` to accept both `IRequest<T>` (Promise<T>) and `IRequestHandler<T>`. Includes `PromiseRequestHandler` adapter and `toRequestHandler` helper for backward compatibility. Allows swapping implementations (real API, mocks, cached) without changing core. Documented in API.md and core README. |
