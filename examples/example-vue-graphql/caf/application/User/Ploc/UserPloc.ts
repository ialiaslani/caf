        ...
        if ((error as Error & { fieldErrors: Record<string, string> }).fieldErrors) {
          ...
          errorParts.forEach((part: string) => {
            ...
            if (trimmed.includes(':')) {
              ...
              validationErrors[field!.trim().toLowerCase()] = messageParts.join(':').trim();
            }
          });
        }
        this.changeState({ ...this.state, loading: false, error: errorMessage, validationErrors });
        return;
      }
      const newUser = useCaseResult.data.value;
      if (!newUser || !newUser.id || !newUser.name?.trim() || !newUser.email?.trim()) {
        this.changeState({
          ...this.state,
          loading: false,
