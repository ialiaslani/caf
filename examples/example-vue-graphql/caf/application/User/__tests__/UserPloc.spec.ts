...
import type { UserState } from '../Ploc/UserPloc';
import { UserPloc } from '../Ploc/UserPloc';
...
describe('UserPloc', () => {
  ...
  describe('loadUsers', () => {
    it('updates state with users on success', async () => {
      ...
      ploc = new UserPloc(getUsers as never, createUser as never);
  ...
  describe('createUser', () => {
    ...
    it('sets validation errors when CreateUser returns validation error', async () => {
      ...
      ploc = new UserPloc(createMockUseCaseSuccess(mockUsers) as never, createUser as never);
      tester = createPlocTester(ploc);
  ...
  describe('selectUser / clearError', () => {
    ...
    it('clearError clears error', async () => {
      ...
      ploc = new UserPloc(getUsersErr as never, createMockUseCaseSuccess(mockUsers[0]) as never);
      await ploc.loadUsers();
